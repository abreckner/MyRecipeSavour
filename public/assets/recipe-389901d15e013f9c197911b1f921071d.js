...
(function() {
  var add_ingredient_handler, delete_ingredient_handler, ingredient_row_template, ingredient_select_handler, save_ingredient;

  ingredient_select_handler = function() {
    var amount, amount_text, caret, content, first_part, ingredient, ingredient_id, original_value, second_part;
    ...
    if (amount.val() === '') {
      ...
      $(this).val(content);
      return $.ajax("/ingredients/" + ingredient_id + ".json", {
        type: 'PUT',
        dataType: 'JSON',
        data: {
          'ingredient': {
  ...
  add_ingredient_handler = function() {
    return $('.new_ingredient input').show();
  };

  save_ingredient = function() {
    var amount, ingredient, ingredient_type, recipe_id;
    ingredient = $(this).parents('.new_ingredient');
    amount = ingredient.find('.amount').val();
    ingredient_type = $(this).val();
    recipe_id = $('#recipe_id').val();
    return $.ajax("/ingredients.json", {
      type: 'POST',
      dataType: 'JSON',
      data: {
        'ingredient': {
          'content': ingredient_type,
          'amount': amount
        },
        'recipe_id': recipe_id
      },
      ...
      success: function(data, textStatus, jqXHR) {
        ...
        ingredients_table.append(_.template(ingredient_row_template, {
          ingredient: data
        }));
        $('.new_ingredient input').hide();
        return $('.new_ingredient input').val('');
      }
    });
  };

  delete_ingredient_handler = function() {
    var ingredient_id;
    ...
    return $.ajax("/ingredients/" + ingredient_id + ".json", {
      ...
      success: function(data, textStatus, jqXHR) {
        $('body').append("Successful AJAX call: " + data);
        return $(".ingredient[data-ref=" + ingredient_id + "]").hide('fast', function() {
          return $(this).remove();
        });
      }
    });
  ...
  $(function() {
    ...
    return $('.delete_ingredient').live('click', delete_ingredient_handler);
  });
